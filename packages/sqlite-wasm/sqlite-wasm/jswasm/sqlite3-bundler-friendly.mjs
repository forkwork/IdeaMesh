    ...
    Module.postRun.push(function (Module) {
      ...
      globalThis.sqlite3ApiBootstrap.initializers.push(function (sqlite3) {
        const installOpfsVfs = function callee(options) {
          ...
          const thePromise = new Promise(function (
            ...
            const W = new Worker(
              new URL('sqlite3-opfs-async-proxy.js', _scriptDir),
            );
            setTimeout(() => {
      ...
      globalThis.sqlite3ApiBootstrap.initializers.push(function (sqlite3) {
        ...
        class OpfsSAHPool {
          constructor(options = Object.create(null)) {
            ...
            _classPrivateFieldSet(
              ...
              _dvBody,
              new DataView(
                _classPrivateFieldGet(this, _apBody).buffer,
                _classPrivateFieldGet(this, _apBody).byteOffset,
              ),
            );
          ...
          log(...args) {
            _classPrivateMethodGet(this, _logImpl, _logImpl2).call(
              this,
              2,
              ...args,
            );
          ...
          async addCapacity(n) {
            for (let i = 0; i < n; ++i) {
              ...
              const h = await _classPrivateFieldGet(
                this,
                _dhOpaque,
              ).getFileHandle(name, {
                create: true,
          ...
          async reduceCapacity(n) {
            let nRm = 0;
            for (const ah of Array.from(
              _classPrivateFieldGet(this, _availableSAH),
            )) {
          ...
          async acquireAccessHandles(clearFiles) {
            const files = [];
            for await (const [name, h] of _classPrivateFieldGet(
              this,
              _dhOpaque,
            )) {
              if ('file' === h.kind) {
              ...
              files.map(async ([name, h]) => {
                try {
                  ...
                  if (clearFiles) {
                    ...
                    if (path) {
                      _classPrivateFieldGet(this, _mapFilenameToSAH).set(
                        path,
                        ah,
                      );
                    } else {
          ...
          getAssociatedPath(sah) {
            ...
            const flags = _classPrivateFieldGet(this, _dvBody).getUint32(
              HEADER_OFFSET_FLAGS,
            );
            if (
              _classPrivateFieldGet(this, _apBody)[0] &&
              (flags & capi.SQLITE_OPEN_DELETEONCLOSE ||
                (flags & PERSISTENT_FILE_TYPES) === 0)
            ) {
              warn(
                `Removing file with unexpected flags ${flags.toString(16)}`,
                _classPrivateFieldGet(this, _apBody),
              );
            ...
            if (fileDigest.every((v, i) => v === compDigest[i])) {
              const pathBytes = _classPrivateFieldGet(this, _apBody).findIndex(
                (v) => 0 === v,
              );
              if (0 === pathBytes) {
              ...
              return pathBytes
                ? textDecoder.decode(
                    _classPrivateFieldGet(this, _apBody).subarray(0, pathBytes),
                  )
                : '';
            } else {
          ...
          setAssociatedPath(sah, path, flags) {
            const enc = textEncoder.encodeInto(
              path,
              _classPrivateFieldGet(this, _apBody),
            );
            ...
            _classPrivateFieldGet(this, _apBody).fill(
              0,
              enc.written,
              HEADER_MAX_PATH_SIZE,
            );
            _classPrivateFieldGet(this, _dvBody).setUint32(
              HEADER_OFFSET_FLAGS,
              flags,
            );
          ...
          async reset(clearFiles) {
            ...
            _classPrivateFieldSet(
              this,
              _dhOpaque,
              await _classPrivateFieldGet(this, _dhVfsRoot).getDirectoryHandle(
                OPAQUE_DIR_NAME,
                {
                  create: true,
                },
          ...
          deletePath(path) {
            const sah = _classPrivateFieldGet(this, _mapFilenameToSAH).get(
              path,
            );
            if (sah) {
          ...
          async removeVfs() {
            if (
              !_classPrivateFieldGet(this, _cVfs).pointer ||
              !_classPrivateFieldGet(this, _dhOpaque)
            )
              return false;
            capi.sqlite3_vfs_unregister(
              _classPrivateFieldGet(this, _cVfs).pointer,
            );
            _classPrivateFieldGet(this, _cVfs).dispose();
            try {
              this.releaseAccessHandles();
              await _classPrivateFieldGet(this, _dhVfsRoot).removeEntry(
                OPAQUE_DIR_NAME,
                {
                  recursive: true,
                },
              ...
              await _classPrivateFieldGet(this, _dhVfsParent).removeEntry(
                _classPrivateFieldGet(this, _dhVfsRoot).name,
                {
                  recursive: true,
                },
              );
              _classPrivateFieldSet(
                this,
                _dhVfsRoot,
                _classPrivateFieldSet(this, _dhVfsParent, undefined),
              );
          ...
          exportFile(name) {
            const sah =
              _classPrivateFieldGet(this, _mapFilenameToSAH).get(name) ||
        ...
        class OpfsSAHPoolUtil {
          ...
          getCapacity() {
            return _classPrivateFieldGet(this, _p).getCapacity(
              _classPrivateFieldGet(this, _p),
            );
          }
          getFileCount() {
          ...
          async reserveMinimumCapacity(min) {
            ...
            return c < min
              ? _classPrivateFieldGet(this, _p).addCapacity(min - c)
              : c;
          }
          exportFile(name) {
