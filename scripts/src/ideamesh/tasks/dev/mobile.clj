...
(defn- set-system-env
  ...
  (let [ip (string/trim (:out (shell {:out :string} "ipconfig getifaddr en0")))
        ideamesh-app-server-url (format "%s://%s:%s" "http" ip "3001")]
    (println "Server URL:" ideamesh-app-server-url)
    ...
    (let [new-body (-> (slurp "capacitor.config.ts")
                       ...
                       (string/replace "process.env.IDEAMESH_APP_SERVER_URL"
                                       (pr-str ideamesh-app-server-url))
