(ns ideamesh.tasks.lang
  "Tasks related to language translations"
  (:require [clojure.set :as set]
            [clojure.string :as string]
            [frontend.dicts :as dicts]
            [ideamesh.tasks.util :as task-util]
            [babashka.cli :as cli]
...
(defn- validate-non-default-languages
  ...
  language English. Ideamesh needs to work out of the box with its default
...
(defn- whiteboard-dicts
  []
  (->> (shell {:out :string}
              "grep -E -oh" "\\bt\\('[^ ']+" "-r" "tldraw/apps/tldraw-ideamesh/src/components")
       :out
       string/split-lines
       (map #(keyword (subs % 3)))))
...
(def allowed-duplicates
  ...
  {:fr #{:port :type :help/docs :search-item/page :shortcut.category/navigating :text/image
   ...
   :it #{:home :handbook/home :host :help/awesome-ideamesh :on-boarding/section-computer
