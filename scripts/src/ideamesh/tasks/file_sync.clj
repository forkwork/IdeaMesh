...
* Wait for test results. Each action takes 10-20s and prints results as it goes"
  (:require [clojure.string :as str]
            ...
            [ideamesh.tasks.file-sync-actions :as file-sync-actions])
...
(def read-config
  ...
  ideamesh.khulnasoft.com/auth_callback request in Network tab, choosing Copy > 'Copy
  response' and saving"
...
(defn- api-get-all-files
  ...
  (let [body (json/generate-string {"GraphUUID" graph-id
                                    "Dir" subdir})
        resp (post "https://api-dev.ideamesh.khulnasoft.com/file-sync/get_all_files"
...
(defn- api-post-get-graphs
  []
  (let [resp (post "https://api-dev.ideamesh.khulnasoft.com/file-sync/list_graphs"
                   {:headers (build-headers)})
        body (json/parse-string (:body resp) keyword)]
    (->> body
         :Graphs
...
(defn- wait&files-are-in-sync?
  ...
  (if msg
    (println msg)
    (println "Wait 10s for ideamesh to pick up changes..."))
  (Thread/sleep 10000)
...
(defn integration-tests
  ...
  * you also need to open ideamesh-app(or yarn electron-watch),
    and open <dir> and start file-sync"
  [& _args]
  (setup-vars)
  (t/run-tests 'ideamesh.tasks.file-sync))
