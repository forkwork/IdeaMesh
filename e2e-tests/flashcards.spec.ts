...
test('flashcard demo', async ({ page, block }) => {
  ...
  await block.mustFill('Why do you add cards? #card #ideamesh')
  ...
  await block.mustFill('How do you create clozes? #card #ideamesh')
  await block.enterNext()
  expect(await block.indent()).toBe(true)
  ...
  await block.mustFill('{{cards [[ideamesh]]}}')
  ...
  expect(await numberLabel.innerText()).toMatch(/\[\[ideamesh\]\]\s+2\/2/)
