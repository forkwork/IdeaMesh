...
test.skip('enabled plugin system default', async ({ page }) => {
  ...
  expect(await page.evaluate(`typeof ideamesh.api.get_current_graph`))
    .toBe('function')
...
test.skip('play a plugin<ideamesh-journals-calendar> from the Marketplace', async ({ page }) => {
  ...
  const goToToday = page.locator('#ideamesh-journals-calendar--goto-today').locator('a.button')
