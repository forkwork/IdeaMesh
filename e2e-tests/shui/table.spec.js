...
test('table can have it\'s version changed via props', async ({ page, block, graphDir }) => {
  ...
  await setPropInFirstBlock(page, block, 'ideamesh.table.version', '2')
...
test('table can configure ideamesh.color::', async ({ page, block, graphDir }) => {
  const pageTitle = await createRandomPage(page)

  // create a v1 table 
  await page.keyboard.type('ideamesh.table.version:: 2')
  await page.keyboard.press('Shift+Enter')
  ...
  await setPropInFirstBlock(page, block, 'ideamesh.color', 'red')

  // check for gradient accent 
  await expect(await page.getByTestId('v2-table-gradient-accent')).toBeVisible()
})

test('table can configure ideamesh.table.hover::', async ({ page, block, graphDir }) => {
  const pageTitle = await createRandomPage(page)
  ...
  await setPropInFirstBlock(page, block, 'ideamesh.table.hover', 'row')
  ...
  await setPropInFirstBlock(page, block, 'ideamesh.table.hover', 'col')
  ...
  await setPropInFirstBlock(page, block, 'ideamesh.table.hover', 'both')
  ...
  await setPropInFirstBlock(page, block, 'ideamesh.table.hover', 'none')
...
test('table can configure ideamesh.table.headers', async ({ page, block, graphDir }) => {
  const pageTitle = await createRandomPage(page)
  ...
  // Check none (explicit)
  await setPropInFirstBlock(page, block, 'ideamesh.table.headers', 'none')
  await expect(await page.getByText('Header A', { exact: true }).innerText()).toEqual("Header A")

  // Check uppercase
  await setPropInFirstBlock(page, block, 'ideamesh.table.headers', 'uppercase')
  await expect(await page.getByText('Header A', { exact: true }).innerText()).toEqual("HEADER A")

  // Check lowercase
  await setPropInFirstBlock(page, block, 'ideamesh.table.headers', 'lowercase')
  await expect(await page.getByText('Header A', { exact: true }).innerText()).toEqual("header a")

  // Check capitalize
  await setPropInFirstBlock(page, block, 'ideamesh.table.headers', 'capitalize')
  await expect(await page.getByText('Header A', { exact: true }).innerText()).toEqual("Header A")

  // Check capitalize-first
  await setPropInFirstBlock(page, block, 'ideamesh.table.headers', 'capitalize-first')
  await expect(await page.getByText('Header A', { exact: true }).innerText()).toEqual("Header a")
})

test('table can configure ideamesh.table.borders', async ({ page, block, graphDir }) => {
  const pageTitle = await createRandomPage(page)
  ...
  // Check true (explicit)
  await setPropInFirstBlock(page, block, 'ideamesh.table.borders', 'true')
  await expect(await page.getByTestId('v2-table-container')).toHaveCSS("gap", /^[1-9].*/)

  // Check false
  await setPropInFirstBlock(page, block, 'ideamesh.table.borders', 'false')
  await expect(await page.getByTestId('v2-table-container')).not.toHaveCSS("gap", /^[1-9].*/)
})

test('table can configure ideamesh.table.stripes', async ({ page, block, graphDir }) => {
  const pageTitle = await createRandomPage(page)
  ...
  await setPropInFirstBlock(page, block, 'ideamesh.table.stripes', 'false')
  await expect(await page.getByText('A1', { exact: true }).getAttribute('class')).toContain("bg-[color:var(--ls-primary-background-color)]")
  await expect(await page.getByText('A2', { exact: true }).getAttribute('class')).toContain("bg-[color:var(--ls-primary-background-color)]")

  // Check false
  await setPropInFirstBlock(page, block, 'ideamesh.table.stripes', 'true')
  await expect(await page.getByText('A1', { exact: true }).getAttribute('class')).toContain("bg-[color:var(--ls-primary-background-color)]")
  await expect(await page.getByText('A2', { exact: true }).getAttribute('class')).toContain("bg-[color:var(--ls-secondary-background-color)]")
})

test('table can configure ideamesh.table.compact', async ({ page, block, graphDir }) => {
  const pageTitle = await createRandomPage(page)
  ...
  // Check false (explicit)
  await setPropInFirstBlock(page, block, 'ideamesh.table.compact', 'false')
  const falseClasses = await page.getByText('A1', { exact: true }).getAttribute('class')

  // Check false
  await setPropInFirstBlock(page, block, 'ideamesh.table.compact', 'true')
...
test('table can configure ideamesh.table.cols::', async ({ page, block, graphDir }) => {
  const pageTitle = await createRandomPage(page)
  ...
  await setPropInFirstBlock(page, block, 'ideamesh.table.cols', 'Header A, Header B')
  await expect(await page.getByText('A1', { exact: true })).toBeVisible()
  await expect(await page.getByText('B1', { exact: true })).toBeVisible()

  await setPropInFirstBlock(page, block, 'ideamesh.table.cols', 'Header A')
  await expect(await page.getByText('A1', { exact: true })).toBeVisible()
  await expect(await page.getByText('B1', { exact: true })).not.toBeVisible()

  await setPropInFirstBlock(page, block, 'ideamesh.table.cols', 'Header B')
