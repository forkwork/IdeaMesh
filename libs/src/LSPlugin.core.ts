...
interface PluginLocalOptions {
  key?: string // Unique from Ideamesh Plugin Store
...
> {
  ...
  async _preparePackageConfigs() {
    ...
    const ideamesh: Partial<LSPluginPkgConfig> = pkg.ideamesh || {}
    ...
    // Entry from main
    const entry = ideamesh.entry || ideamesh.main || pkg.main
    if (validateEntry(entry)) {
      // Theme has no main
      this._options.entry = this._resolveResourceFullUrl(entry, localRoot)
      this._options.devEntry = ideamesh.devEntry

      if (ideamesh.mode) {
        this._options.mode = ideamesh.mode
      }
    }

    const title = ideamesh.title || pkg.title
    const icon = ideamesh.icon || pkg.icon

    this._options.title = title
    this._options.icon = icon && this._resolveResourceFullUrl(icon)
    this._options.theme = Boolean(ideamesh.theme || !!ideamesh.themes)

    // TODO: strategy for Ideamesh plugins center
    if (this.isInstalledInDotRoot) {
      this._id = path.basename(localRoot)
    } else {
      if (ideamesh.id) {
        this._id = ideamesh.id
      } else {
        ideamesh.id = this.id
        try {
...
<html lang="en">
  <head>
    <meta charset="UTF-8">
    <title>ideamesh plugin entry</title>
    ${
        IS_DEV
          ? `<script src="${sdkPathRoot}/lsplugin.user.js?v=${tag}"></script>`
          : `<script src="https://cdn.jsdelivr.net/npm/@ideamesh/libs/dist/lsplugin.user.min.js?v=${tag}"></script>`
      }
...
function setupPluginCore(options: any) {
  ...
  debug('=== 🔗 Setup Ideamesh Plugin System 🔗 ===')
