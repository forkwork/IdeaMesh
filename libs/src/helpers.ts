...
export const URL_LSP = PROTOCOL_LSP + 'ideamesh.io/'
...
export function invokeHostExportedApi(method: string, ...args: Array<any>) {
  ...
  // @ts-ignore
  const nsSDK = window.ideamesh?.sdk
  ...
  const ideameshHostExportedApi = Object.assign(
    // @ts-ignore
    {}, window.ideamesh?.api,
    nsTarget, callables
  )
  ...
  const fn =
    ideameshHostExportedApi[method1] ||
    window.apis[method1] ||
    ideameshHostExportedApi[method] ||
    window.apis[method]
