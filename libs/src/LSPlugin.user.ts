...
const db: Partial<IDBProxy> = {
  ...
  datascriptQuery<T = any> (
    ...
    if (inputs?.some((it) => typeof it === 'function')) {
      ...
      return host.ideamesh.api.datascript_query(query, ...inputs)
    }
...
/**
 * User plugin instance from global namespace `ideamesh`.
 * @example
 * ```ts
 * ideamesh.UI.showMsg('Hello, Ideamesh')
 * ```
 * @public
 */
          ...
          if (tag) {
            ...
            if (hookMatcher != null) {
              ...
              if (typeof handler === 'string' && typeof opts === 'function') {
                handler = handler.replace(/^ideamesh./, ':')
                type = `${type}${handler}`
                handler = opts
                opts = args[2]
              }
...
if (window.__LSP__HOST__ == null) {
  ...
  window.ideamesh = setupPluginUserInstance({} as any, caller)
}
