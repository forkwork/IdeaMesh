...
export class LSPluginExperiments {
  ...
  public invokeExperMethod(type: string, ...args: Array<any>) {
    ...
    return host.ideamesh.api['exper_' + type]?.apply(host, args)
  }
  ...
  registerFencedCodeRenderer(
    ...
    return this.ensureHostScope().ideamesh.api.exper_register_fenced_code_renderer(
      this.ctx.baseInfo.id,
      type,
      opts
    )
  ...
  registerExtensionsEnhancer<T = any>(
    ...
    return host.ideamesh.api.exper_register_extensions_enhancer(
      this.ctx.baseInfo.id,
      type,
      enhancer
    )
