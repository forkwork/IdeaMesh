...
import { IdeameshContext, IdeameshContextValue } from '../lib/ideamesh-context'
...
const handleCreatingShapes = async (
  ...
  async function tryCreateShapesFromDataTransfer(dataTransfer: DataTransfer) {
    return tryCreateShapeHelper(
      ...
      tryCreateIdeameshPortalShapesFromString
    )(dataTransfer)
  }
  ...
  async function tryCreateShapeFromBlockUUID(dataTransfer: DataTransfer) {
    // This is a Ideamesh custom data type defined in frontend.components.block
    const rawText = dataTransfer.getData('block-uuid')
    if (rawText) {
      const text = rawText.trim()
      const allSelectedBlocks = window.ideamesh?.api?.get_selected_blocks?.()
      const blockUUIDs =
        allSelectedBlocks && allSelectedBlocks?.length > 1
          ? allSelectedBlocks.map(b => b.uuid)
          : [text]
      // ensure all uuid in blockUUIDs is persisted
      window.ideamesh?.api?.set_blocks_id?.(blockUUIDs)
      const tasks = blockUUIDs.map(uuid => tryCreateIdeameshPortalShapesFromUUID(`((${uuid}))`))
      ...
      return newShapes.map((s, idx) => {
        ...
        return {
          ...
          point: [point[0] + (IdeameshPortalShape.defaultProps.size[0] + 16) * idx, point[1]],
        }
      })
    }
    return null
  ...
  async function tryCreateShapeFromPageName(dataTransfer: DataTransfer) {
    ...
    if (rawText) {
      ...
      return tryCreateIdeameshPortalShapesFromUUID(`[[${text}]]`)
    }
    return null
  }
  ...
  async function tryCreateIdeameshPortalShapesFromString(item: DataTransfer | ClipboardItem) {
    const rawText = await getDataFromType(item, 'text/plain')
    if (rawText) {
      const text = rawText.trim()
