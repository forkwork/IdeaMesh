...
import { IdeameshPortalShape } from '../../lib'
import { IdeameshContext, SearchResult } from '../../lib/ideamesh-context'
import { CircleButton } from '../Button'
import { TablerIcon } from '../icons'
import { TextInput } from '../inputs/TextInput'

interface IdeameshQuickSearchProps {
...
export const IdeameshQuickSearch = observer(
  ({ className, style, placeholder, onChange, onBlur, onAddBlock }: IdeameshQuickSearchProps) => {
    const [q, setQ] = React.useState(IdeameshPortalShape.defaultSearchQuery)
    const [searchFilter, setSearchFilter] = React.useState<'B' | 'P' | null>(
      IdeameshPortalShape.defaultSearchFilter
    )
    const rInput = React.useRef<HTMLInputElement>(null)
    const { handlers, renderers } = React.useContext(IdeameshContext)
    const t = handlers.t
    ...
    const finishSearching = React.useCallback((id: string) => {
      ...
      rInput.current?.blur()
      if (id) {
        IdeameshPortalShape.defaultSearchQuery = ''
        IdeameshPortalShape.defaultSearchFilter = null
      }
    }, [])
    ...
    React.useEffect(() => {
      IdeameshPortalShape.defaultSearchQuery = q
      IdeameshPortalShape.defaultSearchFilter = searchFilter
    }, [q, searchFilter])
        ...
        options.push({
          ...
          element: (
            <div className="tl-quick-search-option-row">
              <IdeameshTypeTag active type="BA" />
              {q.length > 0 ? (
                <>
        ...
        options.push(
          {
            ...
            element: (
              <div className="tl-quick-search-option-row">
                <IdeameshTypeTag active type="PA" />
          ...
          },
          {
            ...
            element: (
              <div className="tl-quick-search-option-row">
                <IdeameshTypeTag active type="WA" />
        ...
        options.push(
          ...searchResult.pages.map(page => {
            return {
              ...
              element: (
                <div className="tl-quick-search-option-row">
                  <IdeameshTypeTag type={handlers.isWhiteboardPage(page) ? 'WP' : 'P'} />
            ...
            .map(({ content, uuid }) => {
              ...
              return {
                ...
                onChosen: () => {
                  if (block) {
                    finishSearching(uuid)
                    window.ideamesh?.api?.set_blocks_id?.([uuid])
                    return true
                  }
                  return false
                },
        ...
        <div className="tl-quick-search-input-container">
          {searchFilter && (
            <div className="tl-quick-search-input-filter">
              <IdeameshTypeTag type={searchFilter} />
