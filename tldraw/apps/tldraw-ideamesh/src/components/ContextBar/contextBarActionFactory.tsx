...
export const shapeMapping: Record<ShapeType, ContextBarActionType[]> = {
  'ideamesh-portal': ['Swatch', 'IdeameshPortalViewMode', 'ScaleLevel', 'AutoResizing', 'Links'],
...
const AutoResizingAction = observer(() => {
  ...
  const {
    handlers: { t },
  } = React.useContext(IdeameshContext)
  const shapes = filterShapeByAction<IdeameshPortalShape | TextShape | HTMLShape>('AutoResizing')
  ...
  return (
    <ToggleInput
      ...
      toggle={shapes.every(s => s.props.type === 'ideamesh-portal')}
      className="tl-button"
      pressed={pressed}
      onPressedChange={v => {
        shapes.forEach(s => {
          if (s.props.type === 'ideamesh-portal') {
            s.update({
              isAutoResizing: v,
            })
          } else {
...
const IdeameshPortalViewModeAction = observer(() => {
  const app = useApp<Shape>()
  const {
    handlers: { t },
  } = React.useContext(IdeameshContext)
  const shapes = filterShapeByAction<IdeameshPortalShape>('IdeameshPortalViewMode')
...
const ScaleLevelAction = observer(() => {
  ...
  const shapes = filterShapeByAction<IdeameshPortalShape>('ScaleLevel')
  ...
  return (
    <span className="flex gap-3">
      ...
      <Button
        ...
        type="button"
        onClick={() => window.ideamesh?.api?.open_external_link?.(shape.props.url)}
      >
        <TablerIcon name="external-link" />
      </Button>
    </span>
...
const LinksAction = observer(() => {
  ...
  return (
    <ShapeLinksInput
      ...
      side="right"
      pageId={shape.props.type === 'ideamesh-portal' ? shape.props.pageId : undefined}
      portalType={shape.props.type === 'ideamesh-portal' ? shape.props.blockType : undefined}
    />
  )
})
...
contextBarActionMapping.set('IdeameshPortalViewMode', IdeameshPortalViewModeAction)
