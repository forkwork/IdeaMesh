...
const common = {
  ...
  async runCapWithLocalDevServerEntry (cb) {
    ...
    const IP = ip.address()
    const IDEAMESH_APP_SERVER_URL = `http://${IP}:3001`

    if (typeof global.fetch === 'function') {
      try {
        await fetch(IDEAMESH_APP_SERVER_URL)
      } catch (e) {
        return cb(new Error(`/* ❌ Please check if the service is ON. (${IDEAMESH_APP_SERVER_URL}) ❌ */`))
      }
    }

    console.log(`------ Cap ${mode.toUpperCase()} -----`)
    console.log(`Dev serve at: ${IDEAMESH_APP_SERVER_URL}`)
    console.log(`--------------------------------------`)

    cp.execSync(`npx cap sync ${mode}`, {
      stdio: 'inherit',
      env: Object.assign(process.env, {
        IDEAMESH_APP_SERVER_URL
      })
    })
