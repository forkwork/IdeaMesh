(ns frontend.config
  ...
  (:require [clojure.set :as set]
            ...
            [ideamesh.graph-parser.config :as gp-config]
            [ideamesh.graph-parser.util :as gp-util]
...
(if ENABLE-FILE-SYNC-PRODUCTION
  (do (def FILE-SYNC-PROD? true)
      (def LOGIN-URL
        "https://ideamesh-prod.auth.us-east-1.amazoncognito.com/login?client_id=3c7np6bjtb4r1k1bi9i049ops5&response_type=code&scope=email+openid+phone&redirect_uri=ideamesh%3A%2F%2Fauth-callback")
      (def API-DOMAIN "api.ideamesh.khulnasoft.com")
      (def WS-URL "wss://ws.ideamesh.khulnasoft.com/file-sync?graphuuid=%s")
      ...
      (def OAUTH-DOMAIN "ideamesh-prod.auth.us-east-1.amazoncognito.com")
      (def CONNECTIVITY-TESTING-S3-URL "https://ideamesh-connectivity-testing-prod.s3.us-east-1.amazonaws.com/ideamesh-connectivity-testing")
      )

  (do (def FILE-SYNC-PROD? false)
      (def LOGIN-URL
        "https://ideamesh-test2.auth.us-east-2.amazoncognito.com/login?client_id=3ji1a0059hspovjq5fhed3uil8&response_type=code&scope=email+openid+phone&redirect_uri=ideamesh%3A%2F%2Fauth-callback")
      (def API-DOMAIN "api-dev.ideamesh.khulnasoft.com")
      (def WS-URL "wss://ws-dev.ideamesh.khulnasoft.com/file-sync?graphuuid=%s")
      ...
      (def OAUTH-DOMAIN "ideamesh-test2.auth.us-east-2.amazoncognito.com")
      (def CONNECTIVITY-TESTING-S3-URL "https://ideamesh-connectivity-testing-prod.s3.us-east-1.amazonaws.com/ideamesh-connectivity-testing")))
...
;; - `ideamesh_local_/absolute/path/to/graph` => local graph, native fs backend
;; - `ideamesh_local_x:/absolute/path/to/graph` => local graph, native fs backend, on Windows
;; - `ideamesh_local_GraphName` => local graph, browser fs backend
;; - Use `""` while writing global files

(defonce idb-db-prefix "ideamesh-db/")
(defonce local-db-prefix "ideamesh_local_")
...
(defn get-repo-dir
  ...
  (cond
    ...
    ;; Format: ideamesh_local_{dir-name}
    (local-db? repo-url)
