...
;; 1. Graph version, similar to ideamesh.khulnasoft.com
;; 2. Traditional blog version, much faster to load
;; We might host the pages or blocks directly on ideamesh.khulnasoft.com in the future.
...
(defn restore-from-transit-str!
  ...
  (when-let [data js/window.ideamesh_db]
    (let [data (unescape-html data)
          db-conn (d/create-conn db-schema/schema)
          _ (swap! db/conns assoc "ideamesh-db/local" db-conn)
          db (db/string->db data)]
      (reset! db-conn db))))

(defn restore-state!
  []
  (when-let [data js/window.ideamesh_state]
