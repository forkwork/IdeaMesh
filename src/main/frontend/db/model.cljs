(ns frontend.db.model
  ...
  (:require [clojure.set :as set]
            ...
            [ideamesh.db.default :as default-db]
            [ideamesh.db.rules :as rules]
            ...
            [ideamesh.graph-parser.util.db :as db-util]
...
(defn get-custom-css
  []
  (when-let [repo (state/get-current-repo)]
    (get-file repo "ideamesh/custom.css")))
...
(defn get-macro-blocks
  ...
  (d/q
   '[:find [(pull ?b [*]) ...]
     ...
     [(get ?properties :ideamesh.macro-name) ?name]
     [(= ?name ?macro-name)]]
   (conn/get-db repo)
   macro-name))
...
(defn get-whiteboard-id-nonces
  ...
  (->> (get-page-blocks-no-cache repo page-name {:keys [:block/uuid :block/properties]})
       (filter #(:ideamesh.tldraw.shape (:block/properties %)))
       (map (fn [{:block/keys [uuid properties]}]
              {:id (str uuid)
               :nonce (get-in properties [:ideamesh.tldraw.shape :nonce])}))))
