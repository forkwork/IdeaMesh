...
(defn blocks-with-ordered-list-props
  ...
  (let [target-block (if sibling? target-block (some-> target-block :db/id db/pull block tree/-get-down :data))]
    (letfn [(list-type-fn [b] (some-> b :block/properties :ideamesh.order-list-type))]
        ...
        (mapv
          (fn [{:block/keys [content format] :as block}]
            (cond-> block
              ...
              (-> (update :block/properties #(assoc % :ideamesh.order-list-type list-type))
                  (assoc :block/content (property/insert-property format content :ideamesh.order-list-type list-type)))))
          blocks)
        blocks))))
