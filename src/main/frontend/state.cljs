  ...
  (let [document-mode? (or (storage/get :document/mode?) false)
   ...
   (atom
    {:route-match                           nil
     ...
     :ui/loading?                           {}
     :feature/enable-sync?                  (storage/get :ideamesh-sync-enabled)
     :feature/enable-sync-diff-merge?       ((fnil identity true) (storage/get :ideamesh-sync-diff-merge-enabled))
...
(defn active-tldraw-app
  []
  (when-let [tldraw-el (.querySelector js/document.body ".ideamesh-tldraw[data-tlapp]")]
    (gobj/get js/window.tlapps (.. tldraw-el -dataset -tlapp))))

(defn tldraw-editing-ideamesh-block?
  []
...
(defn editing-whiteboard-portal?
  []
  (and (active-tldraw-app) (tldraw-editing-ideamesh-block?)))
...
(defn unset-color-accent! []
  (swap! state assoc :ui/radix-color :ideamesh)
