...
(defn- get-ignored-files
  []
  (into #{#"ideamesh/graphs-txid.edn$"
          #"ideamesh/pages-metadata.edn$"
          #"ideamesh/version-files/"
          #"ideamesh/bak/"
          #"node_modules/"
...
(def ^:private ignore-default-value-files
  ...
  #{"ideamesh/config.edn" "ideamesh/custom.css"})
...
(defn- <apply-remote-deletion
  ...
  (go
    (p->c (p/all (->> relative-paths
                      (map (fn [rpath]
                             (p/let [base-file (path/path-join "ideamesh/version-files/base" rpath)
      ...
      (if (instance? ExceptionInfo fetched-file-rpaths-or-ex)
        ...
        (<!
         (p->c (p/all (->> fetched-file-rpaths-or-ex
                           (map (fn [rpath]
                                  (p/let [incoming-file (path/path-join "ideamesh/version-files/incoming" rpath)
                                          base-file (path/path-join "ideamesh/version-files/base" rpath)
                                          current-change-file rpath
                                          format (gp-util/get-format current-change-file)
                                          repo (state/get-current-repo)
                                          repo-dir (config/get-repo-dir repo)
...
(defn- <connectivity-testing
  []
  (go
    (let [api-url (str "https://" config/API-DOMAIN "/ideamesh/version")
