...
(def backup-dir "ideamesh/bak")
(def version-file-dir "ideamesh/version-files/local")
...
(defn- local-container-path?
  "Check whether `path' is ideamesh's container `localDocumentsPath' on iOS"
...
(rum/defc instruction
  ...
  [:div.instruction
   ...
   [:p.leading-6 "Ideamesh app can only save or access your graphs stored in a specific directory with a "
    [:strong "Ideamesh icon"]
...
(defrecord ^:large-vars/cleanup-todo Capacitorfs []
  ...
  (unlink! [this repo fpath _opts]
    (p/let [repo-dir (config/get-local-dir repo)
            recycle-dir (path/path-join repo-dir config/app-name ".recycle") ;; ideamesh/.recycle
