(ns frontend.fs.diff-merge
  ...
  (:require ["@ideamesh/diff-merge" :refer [attach_uuids Differ Merger]]
...
(defn diff
  "2-ways diff
   ...
   https://github.com/khulnasoft/diff-merge/blob/master/lib/mldoc.ts"
  [base incoming]
  (let [differ (Differ.)]
    (.diff_ideameshMode differ (bean/->js base) (bean/->js incoming))))
...
(defn three-way-merge
  ...
  (let [->ast (fn [text] (if (= format :org)
        ...
        ;; _ (prn "ideamesh diff merge version: " version)
        ;; _ (prn "output:")
