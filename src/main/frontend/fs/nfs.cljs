         ...
         (remove  (fn [file]
                    (let [rpath (string/replace-first (.-webkitRelativePath file) (str root-dir "/") "")
                      ...
                      (or  (string/blank? rpath)
                           ...
                           (string/starts-with? rpath "ideamesh/bak")
                           ; (string/starts-with? rpath "ideamesh/version-files")
...
(defrecord ^:large-vars/cleanup-todo Nfs []
  ...
  (readdir [_this dir]
    ...
    ;; There's no Ideamesh Sync support for nfs. So assume dir is always a repo dir.
    (p/let [repo-url (str "ideamesh_local_" dir)
