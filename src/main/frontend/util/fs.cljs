...
(defn ignored-path?
  ...
  [dir path]
  (let [ignores ["." ".recycle" "node_modules" "ideamesh/bak"
                 "ideamesh/version-files" "ideamesh/graphs-txid.edn"]]
    (when (string? path)
      (or
       ...
       (some #(string/ends-with? path %)
             [".DS_Store" "ideamesh/graphs-txid.edn"])
...
(defn read-graphs-txid-info
  [root]
  (when (string? root)
    (p/let [exists? (fs/file-exists? root "ideamesh/graphs-txid.edn")]
      (when exists?
        (-> (p/let [txid-str (fs/read-file root "ideamesh/graphs-txid.edn")
...
;; Source: https://github.com/khulnasoft/ideamesh/blob/1519e35e0c8308d8db90b2525bfe7a716c4cdf04/src/main/frontend/util.cljc#L930
(defn legacy-dot-file-name-sanity
  [page-name]
  (when (string? page-name)
    ;; Bug in original code, but doesn't affect the result
    ;; https://github.com/khulnasoft/ideamesh/blob/1519e35e0c8308d8db90b2525bfe7a716c4cdf04/src/main/frontend/util.cljc#L892
...
;; Source: https://github.com/khulnasoft/ideamesh/blob/e7110eea6790eda5861fdedb6b02c2a78b504cd9/src/main/frontend/util.cljc#L927
...
;; ideamesh.graph-parser.util (parsing only)
