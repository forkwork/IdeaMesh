(ns frontend.mobile.util
  (:require ["@capacitor/core" :refer [Capacitor registerPlugin ^js Plugins]]
            ...
            ["@ideamesh/capacitor-file-sync" :refer [FileSync]]
...
(defn in-iCloud-container-path?
  "Check whether `path' is ideamesh's iCloud container path on iOS"
  [path]
  (string/includes? path "/iCloud~com~ideamesh~ideamesh/"))

(defn is-iCloud-container-path?
  "Check whether `path' is iCloud container path on iOS"
  [path]
  (re-matches #"/iCloud~com~ideamesh~ideamesh/Documents/?$" path))
