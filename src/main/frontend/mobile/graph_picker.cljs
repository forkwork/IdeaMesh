        ...
        on-create        (fn [input-val]
                           (let [graph-name (util/safe-sanitize-file-name input-val)]
                             (if (string/blank? graph-name)
                               ...
                               ;; create graph directory under Ideamesh document folder (local/icloud)
    ...
    [:div.cp__graph-picker.w-full
     ...
     (case step
       ...
       [:div.flex.flex-col.w-full.space-y-3.faster-fade-in
        ...
        [:div.flex.flex-col
         (when can-ideamesh-sync?
           (toggle-item {:title     "Ideamesh sync"
                         :on?       ideamesh-sync-on?
                         :on-toggle #(set-sync-mode! (if % :ideamesh-sync (if native-icloud? :icloud-sync nil)))}))

         (when (and native-icloud? (not ideamesh-sync-on?))
