...
(rum/defc cards-select
  ...
  (let [cards (db-model/get-macro-blocks (state/get-current-repo) "cards")
        items (->> (map (comp :ideamesh.macro-arguments :block/properties) cards)
      ...
      (if (:global? config)
        [:div.ls-card.content
         ...
         [:div
          ...
          [:img.my-4 {:src "https://docs.ideamesh.khulnasoft.com/assets/2021-07-22_22.28.02_1626964258528_0.gif"}]
          [:p (t :flashcards/modal-welcome-desc-2)
           [:a {:href "https://docs.ideamesh.khulnasoft.com/#/page/Flashcards" :target "_blank"}
