...
(rum/defc chapter-select
  ...
  (let [[open?, set-open?] (rum/use-state false)]
    (rum/use-effect!
      (fn []
        (when-let [^js el (js/document.querySelector "[data-identity=ideamesh-handbooks]")]
...
(defonce discord-endpoint "https://plugins.ideamesh.io/ds")
  ...
  (let [[config _] (r/use-atom *config)
    ...
    [:<>
     ...
     [:div.flex.space-x-3
      ...
      (link-card
        {:class "flex-1" :href "https://discuss.ideamesh.khulnasoft.com"}
        [:div.inner.flex.space-x-1.flex-col
        ...
        (when pane-topic?
          [:a.flex.items-center
           {:aria-label "Copy topic link" :tabIndex "0"
            :on-click   (fn []
                          (let [s (str "ideamesh://handbook/" (:key (second active-pane-state)))]
