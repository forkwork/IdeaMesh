...
(defn- call-api
  "Given a fn name from ideamesh.api, invokes it with the given arguments"
  [fn-name & args]
  (when-not (aget js/window.ideamesh "api" fn-name)
    (throw (ex-info "Api function does not exist" {:fn fn-name})))
  (apply js-invoke (aget js/window.ideamesh "api") fn-name args))
