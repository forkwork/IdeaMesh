                ...
                (let [*themes        (::themes state)
                      ...
                      *total         (::total state)
                      ...
                      themes         (cons {:name        (string/join " " ["Default" (string/capitalize mode) "Theme"])
                                            :url         nil
                                            :description (string/join " " ["Ideamesh default" mode "theme."])
                                            :mode        mode
           ...
           [:div
            ...
            [:div.it.flex.px-3.py-1.5.rounded-sm.justify-between
             ...
             [:div.flex.items-center.text-xs
              [:div.opacity-60 (str (or (:name plg) "Ideamesh") " •")]
      ...
      (let [err-handle
            (fn [^js e]
              (case (keyword (aget e "name"))
                ...
                (notification/show! "Illegal Ideamesh plugin package." :error)
...
(rum/defc remote-readme-display
  ...
  (let [src (str "lsp://ideamesh.khulnasoft.com/marketplace.html?repo=" repo)]
...
(rum/defc panel-tab-developer
  ...
  (ui/button
    (t :plugin/contribute)
    :href "https://github.com/khulnasoft/marketplace"
    :class "contribute"
    :intent "link"
    :target "_blank"))
      ...
      [:p.flex.items-center.space-x-2
       [:span.w-60
        ...
        [:datalist#proxy-test-url-datalist
         [:option "https://api.ideamesh.khulnasoft.com/ideamesh/version"]
         [:option "https://ideamesh-connectivity-testing-prod.s3.us-east-1.amazonaws.com/ideamesh-connectivity-testing"]
               ...
               (when (state/developer-mode?)
                 [{:hr true}
                  ...
                  {:title   [:span.flex.items-center.whitespace-nowrap.gap-1
                             (ui/icon "bug") (t :plugin/open-ideamesh-dir) [:code "~/.ideamesh"]]
                   :options {:on-click
                             #(p/let [root (plugin-handler/get-ls-dotdir-root)]
