  ...
  (let [graph-dir      (config/get-repo-dir repo)
    ...
    [:div.cp__file-sync-related-normal-modal
     ...
     [:h2.text-center.opacity-70.text-xs.leading-5
      "Unfortunately, Ideamesh Sync and iCloud don't work perfectly together at the moment. To make sure"
      [:br]
...
(rum/defc onboarding-welcome-ideamesh-sync
  [close-fn]

  (let [[loading? set-loading?] (rum/use-state false)]
    [:div.cp__file-sync-welcome-ideamesh-sync
     [:span.head-bg
     ...
     [:h1.text-2xl.font-bold.flex-col.sm:flex-row
      ...
      [:span.pl-2.dark:text-white.text-gray-800 "Ideamesh Sync! 👋"]]

     [:h2
      "No more cloud storage worries. With Ideamesh's encrypted file syncing, "
      [:br]
...
(rum/defc onboarding-unavailable-file-sync
  ...
  [:div.cp__file-sync-unavailable-ideamesh-sync
   [:span.head-bg]

   [:h1.text-2xl.font-bold
    [:span.pr-2.dark:text-white.text-gray-800 "Ideamesh Sync"]
...
(rum/defc onboarding-congrats-successful-sync
  ...
  [:div.cp__file-sync-related-normal-modal
   ...
   [:h2.text-center.dark:opacity-70.text-sm.opacity-90
    [:div "By using this graph with Ideamesh Sync you can now transition seamlessly between your different "]
    [:div
   ...
   [:div.cloud-tip.rounded-md.mt-6.py-4
    [:div.items-center.opacity-90.flex.justify-center
     ...
     [:strong "Ideamesh Sync is still in Beta and we're working on a Pro plan!"]]]
...
(defn make-onboarding-panel
  ...
  (fn [close-fn]
    ...
    (case type
      :welcome
      (onboarding-welcome-ideamesh-sync close-fn)
