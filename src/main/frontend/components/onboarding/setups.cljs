...
(rum/defc mobile-intro
  ...
  [:div.mobile-intro
   (cond
     ...
     [:div.px-4
      "You can save them in your local storage, and use Ideamesh Sync or any third-party sync service to keep your notes sync with other devices. "
        ...
        [:ul
         (for [[title label icon]
               [[(t :on-boarding/section-assets) "/assets" "whiteboard"]
                ...
                []
                [(t :on-boarding/section-app) "/ideamesh" "tool"]
                [(t :on-boarding/section-config) "/ideamesh/config.edn"]]]
           (if-not title
             [:li.hr]
             [:li
              {:key title}
...
(defn- lsq-import-handler
  ...
  (let [file (first (array-seq (.-files (.-target e))))
    ...
    (if (or edn? json?)
      (do
        (state/set-state! :graph/importing :ideamesh)
        (let [reader (js/FileReader.)
              import-f (if edn?
