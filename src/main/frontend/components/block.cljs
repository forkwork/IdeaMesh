    ...
    [:div.block-content.inline
     ...
     [:<>
      ...
      (case (:block/warning block)
        :multiple-blocks
        [:p.warning.text-sm "Full content is not displayed, Ideamesh doesn't support multiple unordered lists or headings in a block."]
        nil)]]))
...
(defn- block-drop
  ...
  (when-not (dnd-same-block? uuid)
    (let [block-uuids (state/get-selection-block-ids)
      ...
      (if (seq blocks)
        ;; dnd block moving in current Ideamesh instance
