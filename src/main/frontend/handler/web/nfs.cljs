...
(defn- precheck-graph-dir
  ...
  (when (some #(string/ends-with? (:path %) "/ideamesh/config.edn") files)
    (state/pub-event!
     [:notification/show {:content "It seems that you are trying to open a Ideamesh graph folder with nested graph. Please unlink this graph and choose a correct folder."
                          :status :warning
                          :clear? false}]))
  (when (>= (count files) 10000)
    (state/pub-event!
     [:notification/show {:content "It seems that you are trying to open a Ideamesh graph folder that contains an excessive number of files, This might lead to performance issues."
                          :status :warning
                          :clear? true}])))
