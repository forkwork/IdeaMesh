(ns frontend.handler.repo-config
  ...
  ideamesh/config.edn. In the future it may manage more files. This component
  depends on a repo."
...
(defn- get-repo-config-content
  [repo-url]
  (db/get-file repo-url "ideamesh/config.edn"))
...
(defn read-repo-config
  ...
  (try
    ...
    (catch :default e
      (notification/show! "The file 'ideamesh/config.edn' is invalid. Please reload the app to in order to see the error and fix it." :error)
...
(defn create-config-file-if-not-exists
  "Creates a default ideamesh/config.edn if it doesn't exist"
  [repo-url]
  ...
  (let [repo-dir (config/get-repo-dir repo-url)
    ...
    (p/let [_ (fs/mkdir-if-not-exists dir)]
      (let [default-content config/config-default-content
        ...
        (p/let [file-exists? (fs/create-if-not-exists repo-url repo-dir "ideamesh/config.edn" default-content)]
...
(defn start
  ...
  from repo config. It does not manage the repo directory, ideamesh/, as that is
