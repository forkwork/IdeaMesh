(ns frontend.handler.whiteboard
  ...
  (:require [datascript.core :as d]
            ...
            [ideamesh.graph-parser.whiteboard :as gp-whiteboard]
            [promesa.core :as p]
            [goog.object :as gobj]
            [clojure.set :as set]
...
(defn- build-shapes
  [page-block blocks]
  (let [shapes-index (get-in page-block [:block/properties :ideamesh.tldraw.page :shapes-index])
  ...
  (let [page-entity (model/get-page page-name)
    ...
    {:block/name page-name
     ...
     :block/properties {:ls-type :whiteboard-page
                        :ideamesh.tldraw.page {:id (get-k "id")
                                             :name (get-k "name")
  ...
  (let [assets (js->clj-keywordize (.getCleanUpAssets app))
        ...
        deleted-shapes (when (seq deleted-ids)
                         (->> (db/pull-many repo '[*] (mapv (fn [id] [:block/uuid (uuid id)]) deleted-ids))
                              (map (fn [b]
                                     (get-in b [:block/properties :ideamesh.tldraw.shape])))))
...
(defn transact-tldr-delta! [page-name ^js app replace?]
  (let [tl-page ^js (second (first (.-pages app)))
        ...
        prev-shapes-index (get-in page-block [:block/properties :ideamesh.tldraw.page :shapes-index])
...
(defn add-new-block-portal-shape!
  ...
  (when-let [app (state/active-tldraw-app)]
    (let [^js api (.-api app)
          ...
          shape (->ideamesh-portal-shape block-uuid point)]
...
(defn- get-whiteboard-blocks
  "Given a page, return all the ideamesh blocks (exclude all shapes)"
  [page-name]
  (let [blocks (model/get-page-blocks-no-cache page-name)]
    (remove gp-whiteboard/shape-block? blocks)))

(defn- get-last-root-block
  "Get the last root Ideamesh block in the page. Main purpose is to calculate the new :block/left id"
  [page-name]
...
(defn inside-portal?
  [target]
  (some? (dom/closest target ".tl-ideamesh-cp-container")))
...
(defn update-bindings!
  ...
  (when-let [page (db/entity [:block/name page-name])]
    (let [bindings (get-in page [:block/properties :ideamesh.tldraw.page :bindings])]
      (when (seq bindings)
...
(defn update-shapes-index!
  [^js tl-page page-name]
  (when-let [page (db/entity [:block/name page-name])]
    (let [shapes-index (get-in page [:block/properties :ideamesh.tldraw.page :shapes-index])]
          ...
          (case whiteboard-op
            ...
            (do
              ...
              (when (seq changed-shapes)
                (let [prev-shapes (map (fn [b] (get-in b [:block/properties :ideamesh.tldraw.shape]))
                                       prev-changed-blocks)]
