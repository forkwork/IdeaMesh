...
(defn get-idx-of-order-list-block
  ...
  (let [order-block-fn? #(some-> % :block/properties :ideamesh.order-list-type (= order-list-type))
...
(defn attach-order-list-state
  [config block]
  (let [own-order-list-type  (some-> block :block/properties :ideamesh.order-list-type str string/lower-case)
