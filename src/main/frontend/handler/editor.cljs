(ns ^:no-doc frontend.handler.editor
  (:require [clojure.set :as set]
            ...
            [ideamesh.graph-parser.block :as gp-block]
            [ideamesh.graph-parser.mldoc :as gp-mldoc]
            [ideamesh.graph-parser.property :as gp-property]
            [ideamesh.graph-parser.text :as text]
            [ideamesh.graph-parser.utf8 :as utf8]
            [ideamesh.graph-parser.util :as gp-util]
            [ideamesh.graph-parser.util.block-ref :as block-ref]
            [ideamesh.graph-parser.util.page-ref :as page-ref]
            [promesa.core :as p]
            [rum.core :as rum]
...
(defn get-block-own-order-list-type
  [block]
  (some-> block :block/properties :ideamesh.order-list-type))

(defn set-block-own-order-list-type!
  [block type]
  (when-let [uuid (:block/uuid block)]
    (editor-property/set-block-property! uuid :ideamesh.order-list-type (name type))))

(defn remove-block-own-order-list-type!
  [block]
  (when-let [uuid (:block/uuid block)]
    (editor-property/remove-block-property! uuid :ideamesh.order-list-type)))
...
(defn toggle-blocks-as-own-order-list!
  ...
  (when (seq blocks)
    (let [has-ordered?    (some own-order-number-list? blocks)
          ...
          order-list-prop :ideamesh.order-list-type]
      (if has-ordered?
...
(defn whiteboard?
  ...
  (and (state/whiteboard-route?)
       (.closest (.-activeElement js/document) ".ideamesh-tldraw")))
  ...
  (->> (:block/macros (db/entity (:db/id block)))
       (some (fn [macro]
               (when-let [query-body (and
                                      (= "query" (get-in macro [:block/properties :ideamesh.macro-name]))
                                      (first (:ideamesh.macro-arguments (:block/properties macro))))]
                 (seq (:query
                       (try
