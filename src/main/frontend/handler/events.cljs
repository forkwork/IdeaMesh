    ...
    (when-let [graph-name (or dir-name (:GraphName graph))]
      (let [graph-name (util/safe-sanitize-file-name graph-name)]
        (if (string/blank? graph-name)
          ...
          ;; Create graph directory under Ideamesh document folder (local)
  ...
  (notification/show!
   [:div
    [:div.mb-4
     ...
     [:div
      [:p
       "Check " [:a {:href "https://docs.ideamesh.khulnasoft.com/#/page/ideamesh%20file%20and%20folder%20naming%20rules"
                     :target "_blank"}
                 "Ideamesh file and folder naming rules"]
       " for more details."]
      [:p
       (util/format "To solve this problem, we suggest you quit Ideamesh and update the filename format (on Settings > Advanced > Filename format > click EDIT button)%s to avoid more potential bugs."
