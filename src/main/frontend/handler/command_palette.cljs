...
(defn register
  ...
  ```clojure
  (register
   {:id :document/open-ideamesh-doc
    :desc \"Document: open Ideamesh documents\"
    :action (fn [] (js/window.open \"https://docs.ideamesh.khulnasoft.com/\"))})
  ```

  To add i18n support, prefix `id` with command and put that item in dict.
  Example: {:zh-CN {:command.document/open-ideamesh-doc \"打开文档\"}}"
  [{:keys [id] :as command}]
