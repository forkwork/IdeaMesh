...
;; 1. User changes the config.edn directly in ideamesh.khulnasoft.com (fn: alter-file)
...
(defn load-new-repo-to-db!
  ...
  (let [config (or (when-let [content (some-> (first (filter #(= "ideamesh/config.edn" (:file/path %)) file-objs))
                                              :file/content)]
