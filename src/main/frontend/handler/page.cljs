...
(defn- replace-tag-ref!
  ...
  (let [old-tag (util/format "#%s" old-name)
        new-tag (if (re-find #"[\s\t]+" new-name)
                  ...
                  (str "#" new-name))]
    ;; hash tag parsing rules https://github.com/khulnasoft/mldoc/blob/701243eaf9b4157348f235670718f6ad19ebe7f8/test/test_markdown.ml#L631
...
(defn- rename-nested-pages
  ...
  (let [repo            (state/get-current-repo)
    ...
    (when nested-pages
      ;; rename page "[[obsidian]] is a tool" to "[[ideamesh]] is a tool"
    ...
    (when nested-pages-ns
      ;; rename page "[[obsidian/page1]] is a tool" to "[[ideamesh/page1]] is a tool"
  ...
  ([page-name]
   (if page-name
     (util/copy-to-clipboard!
      (url-util/get-ideamesh-graph-page-url nil (state/get-current-repo) page-name))
