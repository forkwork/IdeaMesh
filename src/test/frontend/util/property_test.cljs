...
(deftest get-visible-ordered-properties
  ...
  (testing "hidden properties"
    (are [x y z expected] (= expected (property/get-visible-ordered-properties x y z))
      ;; page block
      {:ideamesh.order-list-type "number" :foo "bar"}  [:ideamesh.order-list-type :foo] {:pre-block false}
      [[:foo "bar"]]
      ;; normal block
      {:ideamesh.order-list-type "number" :foo "bar"}  [:ideamesh.order-list-type :foo] {:pre-block false}
      [[:foo "bar"]]))
  ...
  (testing "hidden editable properties"
    (are [x y z expected] (= expected (property/get-visible-ordered-properties x y z))
      ...
      [[:foo "bar"]]
      ;; normal block
      {:ideamesh.table.version 2} [:ideamesh.table.version] {:pre-block? false}
      '()
      {:ideamesh.table.version 2 :foo "bar"} [:ideamesh.table.version :foo] {:pre-block? false}
      [[:foo "bar"]])))