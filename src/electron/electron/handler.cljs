  ...
  (if (or (plugin/dotdir-file? path)
    ...
    (try
      ...
      (let [file-name   (-> (string/replace path (str repo-dir "/") "")
                            (string/replace "/" "_")
                            (string/replace "\\" "_"))
            recycle-dir (str repo-dir "/ideamesh/.recycle")
...
(defn- get-graphs-dir
  ...
  (let [dir (if utils/ci?
              ...
              (.join node-path (.homedir os) ".ideamesh" "graphs"))]
    (fs-extra/ensureDirSync dir)
    dir))

(defn- get-graphs
  "Returns all graph names in the cache directory (starting with `ideamesh_local_`)"
  []
...
(defn get-graph-name
  "Given a graph's name of string, returns the graph's fullname.
   E.g., given `cat`, returns `ideamesh_local_<path_to_directory>/cat`
...
(defn- read-txid-info!
  [root]
  (try
    (let [txid-path (.join node-path root "ideamesh/graphs-txid.edn")]
...
(defmethod handle :getIdeameshDotDirRoot []
