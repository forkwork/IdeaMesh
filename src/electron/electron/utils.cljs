(ns electron.utils
  (:require ["@ideamesh/rsapi" :as rsapi]
...
(defn get-ls-dotdir-root
  []
  (let [lg-dir (node-path/join (.getPath app "home") ".ideamesh")]
...
(defn- set-rsapi-proxy
  "Set proxy for Ideamesh Sync(rsapi)"
  [{:keys [protocol host port]}]
  (if (and protocol host port (or (= protocol "http") (= protocol "socks5")))
    (let [proxy-url (str protocol "://" host ":" port)]
...
(defn get-graph-dir
  ...
  [graph-name]
  (when (string/includes? graph-name "ideamesh_local_")
    (string/replace-first graph-name "ideamesh_local_" "")))

(defn get-graph-name
  "reversing `get-graph-dir`"
  [graph-dir]
  (str "ideamesh_local_" graph-dir))

(defn decode-protected-assets-schema-path
  [schema-path]
  (cond-> schema-path
    (string? schema-path)
    (string/replace "/ideamesh__colon/" ":/")))
