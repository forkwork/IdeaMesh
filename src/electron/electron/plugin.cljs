...
(defonce github-api-1 "https://plugins.ideamesh.io/github/api")
...
(defn fetch-specific-release-asset
  ...
  https://github.com/hkgnp/ideamesh-osmmaps-plugin has respective values of 1.5
  and v1.5."
                       ...
                       {:response-transform
                        (fn [res]
                          (if (= 404 (.-status res))
                            ...
                            ;; cases. Previous ideamesh versions did not store the
...
* :repo - A Github repo, not a ideamesh repo, e.g. user/repo"
