(ns electron.core
  (:require [electron.handler :as handler]
            ...
            [electron.utils :refer [*win mac? linux? dev? get-win-from-sender
             ...
             :as utils]
            [electron.url :refer [ideamesh-url-handler]]
            ...
            [ideamesh.publishing.export :as publish-export]))

;; Keep same as main/frontend.util.url
(defonce LSP_SCHEME "ideamesh")
(defonce FILE_LSP_SCHEME "lsp")
(defonce FILE_ASSETS_SCHEME "assets")
(defonce LSP_PROTOCOL (str FILE_LSP_SCHEME "://"))
(defonce PLUGIN_URL (str LSP_PROTOCOL "ideamesh.io/"))
(defonce STATIC_URL (str LSP_PROTOCOL "ideamesh.khulnasoft.com/"))
(defonce PLUGINS_ROOT (.join node-path (.homedir os) ".ideamesh/plugins"))
...
(defn setup-updater! [^js win]
  ;; manual/auto updater
  (when-not linux?
    (init-updater {:repo   "ideamesh/ideamesh"
                   :win    win})))
...
(defn open-url-handler
  ...
  (when-let [parsed-url (try (js/URL. url)
    ...
    (when (= (str LSP_SCHEME ":") (.-protocol parsed-url))
      (ideamesh-url-handler win parsed-url))))
...
(defn- set-app-menu! []
  (let [about-fn (fn []
                   (.showMessageBox dialog (clj->js {:title "Ideamesh"
                                                     :icon (node-path/join js/__dirname "icons/ideamesh.png")
                       ...
                       (if mac?
                         {:role "help"
                          :submenu [{:label "Official Documentation"
                                     :click #(.openExternal shell "https://docs.ideamesh.khulnasoft.com/")}]}
                         {:role "help"
                          :submenu [{:label "Official Documentation"
                                     :click #(.openExternal shell "https://docs.ideamesh.khulnasoft.com/")}
                                    {:role "about"
                                     :label "About Ideamesh"
                                     :click about-fn}]}))
      ...
      (.on app "ready"
           (fn []
             (let [t0 (setup-interceptor! app)
                   ...
                   _ (reset! *win win)]
               (logger/info (str "Ideamesh App(" (.getVersion app) ") Starting... "))
