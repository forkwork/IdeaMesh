...
- comment in `server` section in **capacitor.config.ts**, and replace `process.env.IDEAMESH_APP_ASERVER_URL` with your `http://your-local-ip-address:3001` (run `ifconfig` to check).
    ```typescript
    server: {
        url: "process.env.IDEAMESH_APP_ASERVER_URL",
        cleartext: true
        } 
    ```
- Working directory: Ideamesh root directory
- Run `yarn && yarn app-watch` from the ideamesh project root directory in terminal.
...
- Run `npx cap open ios` to open Ideamesh project in Xcode, and build the app there.
...
- In Android Studio, open **Run** -> **Run** to run Ideamesh.
- After ideamesh startup in Android virtual device, repl should be able to connect
