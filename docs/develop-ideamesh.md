...
- [Node.js](https://nodejs.org/en/download/) (See [build.yml](https://github.com/khulnasoft/ideamesh/blob/master/.github/workflows/build.yml) for allowed version)  & [Yarn](https://classic.yarnpkg.com/en/docs/install/)
...
``cmd + shift + p`` -> ``Calva: Connect to a Running REPL Server in the Project`` -> ``ideamesh`` -> ``shadow-cljs``->``:app`` ->``localhost:8701``
...
# If you have opened desktop ideamesh, you should close it. Otherwise, this command will fail.
...
the last time you used dev Ideamesh.
