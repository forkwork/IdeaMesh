(ns ideamesh.shui.table.v2
  (:require 
    [clojure.string :as str]
    [ideamesh.shui.util :refer [use-ref-bounding-client-rect use-dom-bounding-client-rect $main-content] :as util]
    [rum.core :as rum]))
...
(defn lsx 
  ...
  current ideamesh theming variables. Should set the prop to the given css variable"
  ([step] (lsx :bg step))
  ([param step]
   (case step 
...
(defn get-columns [block data]
  (->> (or (some-> (get-in block [:block/properties :ideamesh.table.cols])
                   (str/split #", ?"))
...
(rum/defc root
  ...
  (let [;; In order to highlight cells in the same row or column of the hovered cell, 
        ...
        table-opts {; user configurable properties (sometimes with defaults)
                    :color    (get-view-prop* :ideamesh.color color-accent)
                    :headers  (get-view-prop* :ideamesh.table.headers "none")
                    :borders? (get-view-prop* :ideamesh.table.borders true)
                    :compact? (get-view-prop* :ideamesh.table.compact false)
                    :hover    (get-view-prop* :ideamesh.table.hover "cell")
                    :stripes? (get-view-prop* :ideamesh.table.stripes false)
                    :gray     (color->gray (get-in context [:config :ideamesh.color])) 
