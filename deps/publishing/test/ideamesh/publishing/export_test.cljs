(ns ideamesh.publishing.export-test
  (:require [cljs.test :as t :refer [is use-fixtures async]]
            [ideamesh.publishing.test.helper :as test-helper :include-macros true :refer [deftest-async]]
...
(defn- create-ideamesh-graph
  ...
  [dir]
  (fs/mkdirSync (path/join dir "ideamesh") #js {:recursive true})
  (fs/writeFileSync (path/join dir "ideamesh" "config.edn") "{}")
...
(deftest-async create-export-with-css-files
  (create-static-dir "tmp/static")
  (create-ideamesh-graph "tmp/test-graph")
  (fs/writeFileSync "tmp/test-graph/ideamesh/custom.css" ".foo {background-color: blue}")
  (fs/writeFileSync "tmp/test-graph/ideamesh/export.css" ".foo {background-color: red}")
...
(deftest-async create-export-with-js-files
  (create-static-dir "tmp/static")
  (create-ideamesh-graph "tmp/test-graph")
  (fs/writeFileSync "tmp/test-graph/ideamesh/custom.js" "// foo")
