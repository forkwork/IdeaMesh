(ns ^:node-only ideamesh.publishing
  "This node only ns provides api fns for exporting a publishing app"
  (:require [ideamesh.publishing.html :as publish-html]
...
configuration is done through ideamesh/config.edn. There are a few explicit options that
can be passed:
