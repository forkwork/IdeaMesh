(ns ^:node-only ideamesh.graph-parser.cli-test
  ...
  (let [graph-dir "test/docs-0.9.2"
    ...
    (testing "Asts"
      ...
      (is (empty? (remove #(or
                            ...
                            ;; ideamesh files don't have ast
                            ;; could also used gp-config but API isn't public yet
                            (string/includes? (:file %) (str graph-dir "/ideamesh/")))
                          asts))
...
(deftest build-graph-files
  ...
  (fs/mkdirSync (path/join "tmp/test-graph" "ideamesh" "bak"))
