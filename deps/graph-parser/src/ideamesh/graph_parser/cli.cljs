(ns ^:node-only ideamesh.graph-parser.cli
  "Primary ns to parse graphs with node.js based CLIs"
  (:require ["fs" :as fs]
            ["path" :as path]
            [clojure.edn :as edn]
            [ideamesh.khulnasoft.common.graph :as common-graph]
            [ideamesh.common.config :as common-config]
            [ideamesh.graph-parser :as graph-parser]
...
(defn- read-config
  "Reads repo-specific config from ideamesh/config.edn"
  [dir]
...
(defn parse-graph
  ...
  as it can't assume that the metadata in ideamesh/ is up to date. Directory is
