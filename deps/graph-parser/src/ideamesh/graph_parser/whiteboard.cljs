(ns ideamesh.graph-parser.whiteboard
...
(defn block->shape [block]
  (get-in block [:block/properties :ideamesh.tldraw.shape] nil))

(defn page-block->tldr-page [block]
  (get-in block [:block/properties :ideamesh.tldraw.page] nil))

(defn shape-block? [block]
  (= :whiteboard-shape (get-in block [:block/properties :ls-type] nil)))

;; tldraw shape props is now saved into [:block/properties :ideamesh.tldraw.shape]
;; migrate
(defn shape-block-needs-migrate? [block]
  (let [properties (:block/properties block)]
    (and (seq properties)
         (and (= :whiteboard-shape (:ls-type properties))
              (not (seq (get properties :ideamesh.tldraw.shape nil)))))))

(defn page-block-needs-migrate? [block]
  (let [properties (:block/properties block)]
    (and (seq properties)
         (and (= :whiteboard-page (:ls-type properties))
              (not (seq (get properties :ideamesh.tldraw.page nil)))))))

(defn migrate-shape-block [block]
  (if (shape-block-needs-migrate? block)
    (let [properties (:block/properties block)
          properties (assoc properties :ideamesh.tldraw.shape properties)]
...
(defn migrate-page-block [block]
  (if (page-block-needs-migrate? block)
    (let [properties (:block/properties block)
          properties (assoc properties :ideamesh.tldraw.page properties)]
      (assoc block :block/properties properties))
    block))


(defn- get-shape-refs [shape]
  (let [portal-refs (when (= "ideamesh-portal" (:type shape))
...
(defn- with-whiteboard-content
  ...
  {:block/content (case (:type shape)
                    "text" (:text shape)
                    "ideamesh-portal" (if (= (:blockType shape) "P")
