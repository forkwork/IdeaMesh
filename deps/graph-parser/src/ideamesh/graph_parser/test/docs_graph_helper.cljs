(ns ideamesh.graph-parser.test.docs-graph-helper
...
(defn clone-docs-repo-if-not-exists
  ...
  (when-not (.existsSync fs dir)
    (sh ["git" "clone" "--depth" "1" "-b" branch "-c" "advice.detachedHead=false"
         "https://github.com/khulnasoft/docs" dir] {})))
...
(defn- query-assertions
  ...
  (testing "Query based stats"
    (is (= (->> files
                ;; ideamesh files aren't saved under :block/file
    ...
    (is (= {:description 81, :updated-at 46, :tags 5, :ideamesh.macro-arguments 104
            :ideamesh.tldraw.shape 79, :card-last-score 6, :card-repeats 6,
            :card-next-schedule 6, :ls-type 79, :card-last-interval 6, :type 107,
            :template 5, :title 114, :alias 41, :supports 5, :id 145, :url 5,
            :card-ease-factor 6, :ideamesh.macro-name 104, :created-at 46,
    ...
    (is (= {:description 77, :tags 5, :permalink 1, :ls-type 1, :type 104,
            ...
            :ideamesh.tldraw.page 1, :supports 5, :url 5, :platforms 50,
...
(defn docs-graph-assertions
  ...
  ideamesh app. It is important to run these in both contexts to ensure that the
  functionality in frontend.handler.repo and ideamesh.graph-parser remain the
  same"
  [db graph-dir files]
