(ns ideamesh.graph-parser.block
  ...
  (:require [clojure.set :as set]
            ...
            [ideamesh.graph-parser.date-time-util :as date-time-util]
...
(defn- macro->block
  ...
  {:db/ident (str (:name macro) " " (string/join " " (:arguments macro)))
   :block/type "macro"
   :block/properties {:ideamesh.macro-name (:name macro)
                      :ideamesh.macro-arguments (:arguments macro)}})
...
(defn fix-duplicate-id
  [block]
  (println "Ideamesh will assign a new id for this block: " block)
  (-> block
