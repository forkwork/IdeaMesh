(ns ^:node-only ideamesh.khulnasoft.common.graph
...
- Dynamic caches used by Ideamesh are ignored: graph-txid.edn and pages-metadata.edn
- Contents in '**/node_modules/' are ignored
- Contents in '/ideamesh/.recycle/' are ignored
- Contents in '/ideamesh/bak/' are ignored
- Contents in  with '/ideamesh/version-files/' are ignored
"
  [dir path]
    ...
    (when (string? path)
      (or
       (some #(string/starts-with? rpath %)
             ["." "ideamesh/.recycle" "ideamesh/bak" "ideamesh/version-files"])
       (contains? #{"ideamesh/graphs-txid.edn" "ideamesh/pages-metadata.edn"} rpath)
