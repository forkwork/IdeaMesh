...
impl Graph {
        ...
        let futs = {
            walkdir::WalkDir::new(&base_path)
                ...
                .filter(|p| {
                    !(p.starts_with('.')
                        || p.contains("/.")
                        || p.starts_with("ideamesh/bak/")
                        || p.starts_with("ideamesh/version-files/"))
                })
    ...
    pub async fn delete_remote_files<P: AsRef<Path>, S: AsRef<str>>(
        ...
        for file_rpath in &file_paths {
            let _ = fs::remove_file(
                base_path
                    .join("ideamesh/version-files/base")
                    .join(file_rpath),
            )
            .await;
        }
    ...
    /// Ideamesh Sync v2: Fetch remote files to local version DB.
    ...
    pub async fn fetch_remote_files<P: AsRef<Path>, S: AsRef<str>>(
        ...
        for (encrypted_file_path, remote_url) in remote_files {
            ...
            let target_file_path = if is_page_file(&file_path) {
                base_path
                    .join("ideamesh/version-files/incoming")
                    .join(&file_path)
            } else {
    ...
    /// Ideamesh Sync v2, update remote files and save to local version-db.
    pub async fn update_remote_files<P: AsRef<Path>, S: AsRef<str>>(
        ...
        fs::create_dir_all(base_path.join("ideamesh/version-files/base")).await?;
        ...
        tokio::select! {
            task_results = future::join_all(tasks) => {
                ...
                for path in page_files {
                    let target_path = base_path.join("ideamesh/version-files/base").join(&path);
    ...
    pub async fn update_local_version_files<P: AsRef<Path>, S: AsRef<str>>(
        ...
        for (file_id, file_url) in files {
            let full_file_path = base_path.join("ideamesh/version-files").join(&file_id);
